...
steps:
  ...
  - name: Then recover
    text: Use existing commands — dc-up, dc-forward --status, dc-df, dc-prune. Doctor does not run them.
faq:
  ...
  - q: Should agents parse human text?
    a: No. Pass --json. schemaVersion 1. Same 17 check ids in fixed order.
---
...
Checks are a closed list of 17 ids (bash, common library, Docker, Colima, official CLI, workspace path, duplicate labels, stack, desired/actual ports, stale owned sidecars, disk, dc-cli version/channel). Human and JSON are the same opinion.
