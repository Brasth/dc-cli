...
case_json_schema() {
  ...
  for id in platform_bash common_library docker_cli docker_daemon docker_context colima \
    ...
    stack_identity desired_ports required_networks actual_ports stale_owned_sidecars disk \
    dc_cli_version dc_cli_channel dc_cli_update; do
    printf '%s\n' "$out" | grep -q "\"$id\""
  done
  harness_teardown
}
...
run_case "json schema + 19 ids" case_json_schema
