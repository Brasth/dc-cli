...
## Procedure
1. Confirm Docker/Colima (read-only `dc-doctor` or `docker ps`). `dc-doctor` never mutates. On Docker Desktop, run `dc-doctor` **before** `dc-up` — `docker_context` must be one live engine (CLI socket). Split-brain (Colima + Desktop both answering) is a blocker; unset `DOCKER_HOST`, `docker context use desktop-linux` or `colima`, stop the extra engine. `DC_UP_ALLOW_SPLIT=1` is a last-resort hatch only. This folder is `kind=devcontainer` if project config exists (official CLI required), else `kind=compose` if a root compose file exists (official CLI not required), else not a workspace.
...
- One engine per machine. Desktop UI is not proof the CLI is talking to that VM. Do not export Colima `DOCKER_HOST` onto a Desktop laptop.
