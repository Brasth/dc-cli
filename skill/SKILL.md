...
7. Open: `dc-open` host; `--attach` VS Code only. Zed: `dc-up` then `dc-open`, then Project: Open Remote → Connect Dev Container. Zed does not publish `forwardPorts` (`dc-forward` does).
...
- Sublime cannot attach. Zed attach is first-party (not `dc-open --attach`). Native Windows unsupported.
