...
5. Exec: `dc-exec` = labeled **app** (devcontainer exec). Siblings: `dc-exec --list`, `dc-exec --service db` / `--id`. TUI **e** = app; click a stack row for db/mailpit/wordpress. List: `dc-ls --json` / `dc-ps`.
