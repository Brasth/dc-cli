...
## Procedure
1. Confirm Docker/Colima (read-only `dc-doctor` or `docker ps`). `dc-doctor` never mutates. This folder is `kind=devcontainer` if project config exists (official CLI required), else `kind=compose` if a root compose file exists (official CLI not required), else not a workspace.
